...
for (let i = 0; i < clickAreas.length; i++) clickAreas[i].addEventListener('click', (e) => {
  let num = e.target.id;
  ...
  if (move === 9) {
    let yes = alert('no winner!');
    if (yes === true) restartGame();
  }
  else if (move % 2 !== 0) {
    gamer = 'O'
  } else gamer = 'X';
...
const restartGameByButton = () => restart.addEventListener('click', () => restartGame());
